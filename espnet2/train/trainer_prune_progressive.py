...
class Trainer:
    ...
    def run(
        ...
        target_prune_rate = prune_rate*100
        init_zero_rate = see_weight_rate(model)
        per_epoch_increment_prune_rate = 1. * (target_prune_rate - init_zero_rate) / (trainer_options.max_epoch - 1)
        if not distributed_option.distributed or distributed_option.dist_rank == 0:
            # do this only on 1 GPU
            logging.info('model init zero rate is %.2f' % init_zero_rate)
            logging.info('and will be progressively pruned to %.2f' % target_prune_rate) 
            logging.info('prune %.2f each epoch for %d epoches' % (per_epoch_increment_prune_rate, trainer_options.max_epoch))
        current_zero_rate = init_zero_rate
        ...
        for iepoch in range(start_epoch, trainer_options.max_epoch + 1):
            ...
            with reporter.observe("train") as sub_reporter:
                ...
                if not distributed_option.distributed or distributed_option.dist_rank == 0:
                    ...
                    if current_zero_rate < (target_prune_rate - per_epoch_increment_prune_rate):
                        current_zero_rate += per_epoch_increment_prune_rate
                        pruning_transformer_tts(model, current_zero_rate*0.01)
                        zero_rate = see_weight_rate(model)
                        logging.info('model current zero rate is %.2f' % zero_rate)
                        ...
                        current_zero_rate = target_prune_rate
                        pruning_transformer_tts(model, target_prune_rate*0.01)
                        zero_rate = see_weight_rate(model)
                        logging.info('model current zero rate is %.2f' % zero_rate)
                        logging.info('unprune')
                        unprune_transformer_tts(model)
