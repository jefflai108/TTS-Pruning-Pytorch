...
sys.path.append('/data/sls/temp/clai24/lottery-ticket/espnet/egs2/ljspeech/tts1/src')
from pruning_methods import *

# for teacher loss
teacher_before_outs_l2_criterion = torch.nn.MSELoss(reduction="mean")
teacher_after_outs_l2_criterion  = torch.nn.MSELoss(reduction="mean")
teacher_logits_l2_criterion      = torch.nn.MSELoss(reduction="mean")
...
class Trainer:
    ...
    def run(
        ...
        def undistribute_model(dp_model):
            """ 
            undistribute the model from torch.nn.parallel.DataParallel to modify model 
            source: https://discuss.pytorch.org/t/changing-model-structure-in-nn-dataparallel/91261/3
            """
        ...
        dp_model = distribute_model(model)
        ...
        if not distributed_option.distributed or distributed_option.dist_rank == 0:
            # do this only on 1 GPU
            zero_rate = see_weight_rate(model)
            logging.info('model current zero rate is %.2f' % zero_rate)
            logging.info('PARP first step (prior to training):')
            logging.info('directly prune pre-trained model')
            pruning_transformer_tts(model, prune_rate)
            zero_rate = see_weight_rate(model)
            logging.info('model current zero rate is %.2f' % zero_rate)
            logging.info('unprune')
            unprune_transformer_tts(model)
            zero_rate = see_weight_rate(model)
            logging.info('model current zero rate is %.2f' % zero_rate)
        ...
        for iepoch in range(start_epoch, trainer_options.max_epoch + 1):
            ...
            with reporter.observe("train") as sub_reporter:
                ...
                if not distributed_option.distributed or distributed_option.dist_rank == 0:
                    ...
                    zero_rate = see_weight_rate(model)
                    logging.info('model current zero rate is %.2f' % zero_rate)
                    logging.info('at the end of one epoch: PARP adjust and re-prune')
                    pruning_transformer_tts(model, prune_rate)
                    zero_rate = see_weight_rate(model)
                    logging.info('model current zero rate is %.2f' % zero_rate)
                    logging.info('unprune')
        ...
        for iiter, (_, batch) in enumerate(
            ...
            with autocast(scaler is not None):
                with reporter.measure_time("forward_time"):
                    retval  = model(**batch)
                    retval2 = teacher_model(**batch) 
                    # student stuff 
                    loss, stats, weight = retval[0], retval[1], retval[2]
                    olens, after_outs, before_outs, logits, att_ws = retval[-5], retval[-4], retval[-3], retval[-2], retval[-1]
                    stats.update(student_loss=loss.item())
                    # teacher stuff 
                    after_outs_teacher, before_outs_teacher, logits_teacher, att_ws_teacher = \
                        retval2[-4], retval2[-3], retval2[-2], retval2[-1]
                    #logging.info(retval[-3].shape) # torch.Size([5, 376, 80])
                    #logging.info(retval2[-3].shape) # torch.Size([5, 376, 80])
                    #logging.info(retval[-4].shape) # torch.Size([5, 376, 80])
                    #logging.info(retval2[-4].shape) # torch.Size([5, 376, 80])

                    #logging.info(logits.shape) #torch.Size([5, 376])
                    #logging.info(att_ws.shape) #torch.Size([5, 4, 376, 42])
                    #logging.info(logits_teacher.shape)
                    #logging.info(att_ws_teacher.shape)

                    ## Reference espnet.nets.pytorch_backend.e2e_tts_tacotron2.Tacotron2Loss ##
                    masks = make_non_pad_mask(olens).unsqueeze(-1).to(before_outs.device)
                    teacher_before_outs_l2_loss = teacher_before_outs_l2_criterion(before_outs.masked_select(masks), before_outs_teacher.masked_select(masks))
                    teacher_after_outs_l2_loss = teacher_after_outs_l2_criterion(after_outs.masked_select(masks), after_outs_teacher.masked_select(masks))
                    teacher_logits_l2_loss = teacher_logits_l2_criterion(logits.masked_select(masks[:, :, 0]), logits_teacher.masked_select(masks[:, :, 0]))
                    
                    stats.update(teacher_before_outs_l2_loss=teacher_before_outs_l2_loss.item())
                    stats.update(teacher_after_outs_l2_loss=teacher_after_outs_l2_loss.item())
                    stats.update(teacher_logits_l2_loss=teacher_logits_l2_loss.item())

                    teacher_loss = teacher_before_outs_l2_loss + teacher_after_outs_l2_loss + teacher_logits_l2_loss    
                    #print('teacher_before_outs_l2_loss is', teacher_before_outs_l2_loss)
                    #print('teacher_after_outs_l2_loss is', teacher_after_outs_l2_loss)
                    #print('teacher_logits_l2_loss is', teacher_logits_l2_loss)

                    loss = loss + teacher_loss # + KD loss from teacher 
                    stats.update(teacher_loss=teacher_loss.item())
    ...
    def validate_one_epoch(
        ...
        for (_, batch) in iterator:
            ...
            if isinstance(retval, dict):
                ...
                loss, stats, weight = retval[0], retval[1], retval[2]
            stats, weight = force_gatherable((stats, weight), loss.device)
