...
def run_utterance(utt, sparsity, utt_cnt, diff_tuple):
    ...
    unpruned_file = os.path.join(EXPDIR, 'exp/ljspeech_tts_train_transformer_raw_phn_tacotron_g2p_en_no_space/tts_train_transformer_raw_phn_tacotron_g2p_en_no_space/decode_valid.loss.best/dev/wav_parallel_wavegan.v3/', utt + '_gen.wav') 
    ...
    #pruned_file = os.path.join(EXPDIR, 'exp/ljspeech_tts_train_transformer_raw_phn_tacotron_g2p_en_no_space/', 'tts_pretrained-valid.loss.best-prune_at_' + str(sparsity), 'decode_valid.loss.best/dev/wav_parallel_wavegan.v3/', utt + '_gen.wav')     
    ...
    utt_cnt += 1
    unpruned_utt_duration, (_, unpruned_f0_std) = extract_f0_from_audio(unpruned_file)
    #unpruned_utt_duration, (_, unpruned_f0_std) = extract_f0_from_audio(pruned_file)
    utt_duration_diff = unpruned_utt_duration - gt_utt_duration
    #diff_tuple = (diff_tuple[0] + utt_duration_diff, diff_tuple[1] + unpruned_f0_std)
    diff_tuple = (diff_tuple[0] + utt_duration_diff, diff_tuple[1] + gt_f0_std)
